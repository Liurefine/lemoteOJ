...
foreach ($result as $row){
	...
	if (isset($sub_arr[$row['problem_id']])){
		if (isset($acc_arr[$row['problem_id']])) 
			$view_problemset[$i][0]="<div class='label label-success'>正确</div>";
		else 
			$view_problemset[$i][0]= "<div class='label label-danger'>错误</div>";
	}else{
		$view_problemset[$i][0]= "<div class='label label-default'>未作答</div>";
	}
